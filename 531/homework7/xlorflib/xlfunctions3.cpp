...
LPXLFOPER EXCEL_EXPORT xlOrfWorstOfDigitalBSMC(LPXLFOPER xlPayoffType,
	LPXLFOPER xlStrike,
	LPXLFOPER xlFixingTimes,
	LPXLFOPER xlSpots,
	LPXLFOPER xlDiscountCrv,
	...
	LPXLFOPER xlHeaders)
{
	...
	int n_assets = (int)vols.size();

	// create the product
	SPtrProduct spprod(new WorstOfDigitalCallPut(payoffType, strike, fixingTimes, n_assets));
	// create the pricer
	MultiAssetBsMcPricer bsmcpricer(spprod, spyc, divYields, vols, spots, correlMat, mcparams);
	// create the statistics calculator
	MeanVarCalculator<double *> sc(bsmcpricer.nVariables());
