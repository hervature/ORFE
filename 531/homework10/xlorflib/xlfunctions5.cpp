	...
	LPXLFOPER xlHeaders)
{
	...
	int nLambdaSteps = XlfOper(xlNLambdaSteps).AsInt();
	...
	RW offset = headers ? 1 : 0;
	XlfOper xlRet(offset + nLambdaSteps + 1, 3); // construct a range of size 1 x ngreeks
	if (headers) {
		xlRet(0, 0) = "MeanReturn";
		xlRet(0, 1) = "Volatility";
		xlRet(0, 2) = "Lambda";
	...
	for (int i = 0; i <= nLambdaSteps; i++) {
		std::tuple<double, double, double> res = meanVarFront(assetRets, assetVols, correlMat, i*lambdaMax/nLambdaSteps);
