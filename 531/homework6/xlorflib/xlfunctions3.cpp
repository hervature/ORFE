...
LPXLFOPER EXCEL_EXPORT xlOrfEuroBSMC(LPXLFOPER xlPayoffType,
									 ...
									 LPXLFOPER xlVolatilityTermStructure,
                                     ...
                                     LPXLFOPER xlHeaders)
{
  ...
  // get volatility structure

  SPtrVolatilityTermStructure volatilityTermStructure;

  if (XlfOper(xlVolatilityTermStructure).IsString())
  {
	  std::string name = xlStripTick(XlfOper(xlVolatilityTermStructure).AsString());
  ...
  else
  {
	  ...
	  Vector tmats = { 1 };
	  Vector vals = { XlfOper(xlVolatilityTermStructure).AsDouble() };
  ...
  // create the pricer
  BsMcPricer bsmcpricer(spprod, spyc, divYield, spot, volatilityTermStructure, mcparams);
