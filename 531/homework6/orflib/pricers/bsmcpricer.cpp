...
BsMcPricer::BsMcPricer(SPtrProduct prod,
                       ...
                       McParams mcparams)
: prod_(prod), discyc_(discountCurve), divyld_(divYield),
spot_(spot), vol_(volatilityTermStructure), mcparams_(mcparams)
{
  ...
  for (size_t i = 0; i < fixtimes.size(); ++i) {
    ...
    double var = vol_->fwdVol(t1, t2) * vol_->fwdVol(t1, t2) * (t2 - t1);
    stdevs_[i] = sqrt(var);
