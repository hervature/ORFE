	...
	LPXLFOPER xlHeader)
{
	...
	// output = (price, delta, gamma, theta, vega)
	orf::Vector output = europeanOptionBS(payoffType, spot, strike, timeToExp, intRate, divYield, vol);

	if (header) {
		RW n1 = 2;
		COL n2 = output.n_rows;
		XlfOper ret(n1, n2);

		// Set the header
		ret.SetElement(0, 0, "Price");
		ret.SetElement(0, 1, "Delta");
		ret.SetElement(0, 2, "Gamma");
		ret.SetElement(0, 3, "Theta");
		ret.SetElement(0, 4, "Gamma");

		for (unsigned int i = 0; i < output.n_rows; i++) {
			ret.SetElement(1, i, output(i));
		}
